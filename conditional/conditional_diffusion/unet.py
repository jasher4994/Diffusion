...
class ResBlock(nn.Module):
    """
    Residual block that processes images and incorporates time embeddings.
    
    This is a fundamental building block used throughout the UNet.
    """
...
class UNet(nn.Module):
    def __init__(
        ...
        self.bottleneck = nn.ModuleList([
            ResBlock(base_channels * 8, base_channels * 8, time_emb_dim * 4),
            ResBlock(base_channels * 8, base_channels * 8, time_emb_dim * 4)
        ])
        ...
        self.decoder3 = nn.ModuleList([
            ResBlock(base_channels * 8, base_channels * 4, time_emb_dim * 4),  # 8 because of skip connection
            ResBlock(base_channels * 4, base_channels * 4, time_emb_dim * 4)
        ])

        self.up2 = nn.ConvTranspose2d(base_channels * 4, base_channels * 2, 3, stride=2, padding=1, output_padding=1)
        self.decoder2 = nn.ModuleList([
            ResBlock(base_channels * 4, base_channels * 2, time_emb_dim * 4),  # 4 because of skip connection
            ResBlock(base_channels * 2, base_channels * 2, time_emb_dim * 4)
        ])

        self.up1 = nn.ConvTranspose2d(base_channels * 2, base_channels, 3, stride=2, padding=1, output_padding=1)
        self.decoder1 = nn.ModuleList([
            ResBlock(base_channels * 2, base_channels, time_emb_dim * 4),      # 2 because of skip connection
            ResBlock(base_channels, base_channels, time_emb_dim * 4)
        ])
    ...
    def forward(self, x, timesteps, text_embeddings=None):
        ...
        time_emb = self.time_mlp(time_emb)  # Shape: (batch, time_emb_dim * 4)
        
        # Step 2: Initial convolution
        h = self.initial_conv(x)  # Shape: (batch, base_channels, height, width)
        ...
        for block in self.encoder1:
            h = block(h, time_emb)
        skip_connections.append(h)  # Save for skip connection
        h = self.down1(h)  # Downsample
        
        # Encoder level 2
        for block in self.encoder2:
            h = block(h, time_emb)
        skip_connections.append(h)  # Save for skip connection
        h = self.down2(h)  # Downsample
        
        # Encoder level 3
        for block in self.encoder3:
            h = block(h, time_emb)
        skip_connections.append(h)  # Save for skip connection
        h = self.down3(h)  # Downsample
        ...
        # Decoder level 3
        h = self.up3(h)  # Upsample
        h = torch.cat([h, skip_connections.pop()], dim=1)  # Concatenate skip connection
        for block in self.decoder3:
            h = block(h, time_emb)
        
        # Decoder level 2
        h = self.up2(h)  # Upsample
        h = torch.cat([h, skip_connections.pop()], dim=1)  # Concatenate skip connection
        for block in self.decoder2:
            h = block(h, time_emb)
        
        # Decoder level 1
        h = self.up1(h)  # Upsample
        h = torch.cat([h, skip_connections.pop()], dim=1)  # Concatenate skip connection
